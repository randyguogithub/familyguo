    ...
    .content
      .photo
        img.im(
          v-if='photoUrl',
          :src='photoUrl',
          @error='markPhotoNotFound(showPerson)'
        )
      ...
      .details
        table.tab
            ...
            tr(v-for='prop in config.personProperties')
              ...
              td
                template(v-if='!editMode')
                  a(
                    v-if='prop.type == "url"',
                    :href='getPropName(prop)',
                    target='_blank'
                  ) {{ getPropName(prop) }}
                  a(
                    v-else-if='prop.type == "email"',
                    :href='"mailto: " + getPropName(prop)'
                  ) {{ getPropName(prop) }}
                  span(v-else) {{ getPropName(prop) }}
                template(v-else)
                  input(
                    type='text',
                    :value='getPropName(prop)',
                    @input='setPropName(prop, $event.target.value)'
                  )
          ...
          tr(v-if='!showPerson.new')
            ...
            td.val
              .dep(
                v-for='assignment in personAssignments',
                @click='gotoDepartment(assignment.department)'
              ) 
      ...
      div
        button.btn1(
          v-if='showPerson.new',
          @click='addEmployee(showPerson)',
          :disabled='!employeeID || !employeeName'
        ) ADD
        button.btn2(
          v-if='editMode',
          @click='deleteEmployee1(showPerson)',
          title='Removes the user from all departments and completely deletes the user'
        ) Delete this user
</template>
